...
class LASEM(nn.Module):
    ...
    adaptive attention, then added back to the input through a residual connection. The module reduces channels with a
    1x1 convolution, extracts local context in parallel with depthwise 3x3 and 5x5 convolutions, derives a global
    ...
    Attributes:
        ...
        context (nn.Sequential): Global average pooling followed by a 1x1 convolution and activation that summarizes the
            fused branch response into a global context vector.
